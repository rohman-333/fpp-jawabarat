...
export function SuggestedProducts() {
  ...
  useEffect(() => {
    ...
    async function fetchProducts() {
      try {
        ...
        console.error('[SUGGESTED_PRODUCTS_ERR]', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading && products.length === 0) {
    return (
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200 mt-6">
        <h3 className="font-bold text-slate-800 text-sm mb-4">Produk Pilihan</h3>
  ...
  if (products.length === 0) {
    return (
      <div className="bg-white rounded-2xl p-4 text-center border border-slate-100 shadow-sm mt-6">
        <p className="text-slate-400 text-xs font-semibold">Tidak ada produk pilihan baru.</p>
      </div>
    );
  }

  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mt-6 xl:mt-6 w-full">
      <div className="p-3 xl:p-4 border-b border-slate-100 flex items-center gap-2">
        <Store className="w-4 h-4 text-blue-600" />
        <h3 className="font-bold text-slate-800 text-sm">Produk Pilihan</h3>
      </div>
      <div className="p-3 xl:p-4 flex xl:flex-col gap-4 overflow-x-auto xl:overflow-visible hide-scrollbar snap-x w-full">
        {products.map(p => (
          <Link 
            ...
            className="flex flex-col xl:flex-row items-center gap-2 xl:gap-3 group min-w-[140px] xl:min-w-0 p-3 xl:p-0 border border-slate-100 xl:border-none rounded-xl snap-center shrink-0 bg-slate-50/30 xl:bg-transparent"
          >
            <div className="w-16 h-16 xl:w-12 xl:h-12 rounded-lg bg-slate-100 shrink-0 overflow-hidden border border-slate-200">
              {p.image_url ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-slate-300">
                  <Store className="w-5 h-5 xl:w-5 xl:h-5" />
                </div>
              )}
            </div>
            <div className="flex-1 min-w-0 text-center xl:text-left w-full">
              <h4 className="font-bold text-slate-800 text-xs xl:text-sm group-hover:text-blue-600 truncate transition-colors">
                {p.name}
              </h4>
