    ...
    <aside className="w-80 bg-transparent hidden xl:block h-screen sticky top-0 px-6 py-8 overflow-y-auto">
      ...
      <div className="relative mb-8">
        <input 
          ...
          className="w-full bg-white border-0 shadow-sm rounded-full py-3 pl-5 pr-10 text-sm focus:ring-2 focus:ring-blue-500 outline-none"
        />
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 mb-6">
        <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
          <TrendingUp className="w-4 h-4 text-blue-500" /> Sedang Tren
        </h3>
        <div className="space-y-4">
          {[
            ...
            <div key={i} className="group cursor-pointer">
              <p className="font-bold text-slate-700 text-sm group-hover:text-blue-600">#{item.tag}</p>
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
        <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
          <Users className="w-4 h-4 text-blue-500" /> Rekomendasi Pesantren
        </h3>
        <div className="space-y-4">
          {[
            ...
            <div key={i} className="flex items-center gap-3">
              ...
              <div className="flex-1 overflow-hidden">
                <p className="text-sm font-bold text-slate-800 truncate cursor-pointer hover:text-blue-600">{item.name}</p>
                <p className="text-xs text-slate-500">{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
        <Link href="/pesantren" className="block text-center text-sm font-bold text-blue-600 mt-4 hover:underline">
          Lihat Semua
        </Link>
      </div>
