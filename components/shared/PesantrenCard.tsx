    ...
    <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-sm hover:shadow-xl border border-slate-100 transition-all duration-300 hover:-translate-y-1 flex flex-col h-full">
      <div className="flex items-center sm:items-start gap-4 mb-4 sm:mb-5">
        ...
        <div className="flex-1 min-w-0">
          <h2 className="font-bold text-slate-800 text-base sm:text-lg leading-snug mb-1 truncate group-hover:text-emerald-600 transition-colors">{pesantren.name}</h2>
          {pesantren.nspp && (
      ...
      <Link href={`/pesantren/${pesantren.id}`} className="mt-auto block">
        <Button className="w-full bg-emerald-950 text-emerald-50 hover:bg-emerald-900 border-none shadow-md hover:shadow-lg transition-all rounded-xl h-10 sm:h-11 font-semibold text-sm sm:text-base">
